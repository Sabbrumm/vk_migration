class TokenException(Exception):
    ...
    def __init__(self, code, extra=None):
        ...
        elif code == TokenException.CAPTCHA_NEEDED:
            super(TokenException, self).__init__(
                "Captcha needed. Code: {0}. Error extra: {1}".format(code, extrastr))
        elif code == TokenException.STOPPED:
            super(TokenException, self).__init__(
                "Registration stopped. Code: {0}. Error extra: {1}".format(code, extrastr))
        elif code == TokenException.NO_ANSWER:
            super(TokenException, self).__init__(
                "No answer from user. Code: {0}. Error extra: {1}".format(code, extrastr))
